...
(define-filter (decode parser in-port)
    ([π-parser (source (λ () (with-handlers ([exn:fail? die]) (parser in-port))))])
  (λ () (sync (seq-evt (dead-evt π-parser) (λ _ (emit-evt eof)) quit)
              (seq-evt (port-closed-evt in-port) die)
              (seq-evt (recv-evt π-parser)
                (λ (msg)
                  (if (eof? msg)
                      (seq-evt (emit-evt msg) quit)
                      (emit-evt msg))))))
